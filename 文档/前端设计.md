...
- `frontend/`
  ...
  - `src/`
    ...
    - `api/`（Axios 客户端与模块化接口）
      - `http.js`（Axios 实例，统一拦截器，自动注入 `Authorization` 与 `X-User-Id`）
...
- `AuthView` / `UserCenterView`：登录注册、个人信息查看与修改、退出登录 → `POST /api/auth/login`、`POST /api/auth/register`、`GET /api/users/me`、`POST /api/users/me`、`POST /api/auth/logout`
...
- 普通用户页面启用登录守卫：`/product/:id`、`/cart`、`/order/submit`、`/user*`、`/order/:id`
- 未登录访问上述页面时，将重定向至 `/login`，并携带 `redirect=<原始路径>` 参数；登录/注册成功后自动返回原页面
- 管理端页面采用角色守卫：`/admin/**` 的子路由标记 `meta.requiresAdmin=true`，路由守卫依据本地用户信息 `user.role==='admin'` 判定；非管理员访问将重定向到 `/admin/login`
- 顶部导航在普通用户身份下不显示 `Admin` 入口
...
- 开发端口：`10001`（见 `vite.config.js`）；已配置代理将 `'/api'` 指向 `http://localhost:10002`
- 后端端口：`10002`；接口前缀：`http://localhost:10002/api`
